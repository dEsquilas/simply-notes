...
class ImportNotes implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $files = glob($outPath . '/*.html');

        $this->importJob->total_files = count($files);
        $this->importJob->processed_files = 0;
        $this->importJob->status = 'processing';
        $this->importJob->save();

        foreach ($files as $file) {
            $fileContent = file_get_contents($file);
            $this->parseFile($fileContent, $outPath);
            $this->importJob->processed_files++;
            $this->importJob->save();
        }
