...
<template>
    <div data-test="note-list" class="overflow-auto note-list" ref="scrollbar">
        <ul>
            <li v-for="note in notes" :key="note.id" :data-test="'note-' + note.id" v-show="applyFilter(filter, note)" @click="$emit('change-note', note)" @contextmenu="openMenu($event, note)">
